...
89. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII diagnostics keep classifying tokens when multiple non-ASCII whitespace segments appear before the first non-whitespace non-ASCII code point and malformed UTF-8 appears only after that first non-whitespace non-ASCII code point.
