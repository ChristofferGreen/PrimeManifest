...
101. [ ] Add strict-violation reason-token tests verifying malformed-UTF-8-only checks report malformed-UTF-8 diagnostics when multiple malformed UTF-8 segments appear before the first non-whitespace non-ASCII code point and additional malformed plus non-ASCII whitespace segments also appear later.
