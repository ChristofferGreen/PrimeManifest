...
117. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-ASCII-whitespace checks report malformed-UTF-8 diagnostics when multiple non-ASCII whitespace segments appear both before and after the first non-whitespace non-ASCII code point and multiple malformed UTF-8 segments bracket that first code point.
