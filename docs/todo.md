...
88. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII diagnostics classify mixed tokens when non-ASCII whitespace appears before the first non-whitespace non-ASCII code point and malformed UTF-8 appears only after that first non-whitespace non-ASCII code point.
