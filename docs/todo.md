...
123. [ ] Add strict-violation reason-token tests verifying malformed-UTF-8-only checks report malformed-UTF-8 diagnostics when multiple malformed UTF-8 segments appear only before the first non-whitespace non-ASCII code point and multiple non-ASCII whitespace segments appear only before that first code point.
