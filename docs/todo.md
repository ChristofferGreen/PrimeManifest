...
95. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks continue reporting malformed-UTF-8 diagnostics when multiple malformed segments appear both before and after the first non-whitespace non-ASCII code point while multiple non-ASCII whitespace segments are present.
