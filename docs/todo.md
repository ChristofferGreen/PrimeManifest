...
93. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks still report malformed-UTF-8 diagnostics when malformed segments appear both before and after the first non-whitespace non-ASCII code point.
