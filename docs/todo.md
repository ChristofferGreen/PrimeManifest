...
103. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks report malformed-UTF-8 diagnostics when multiple malformed UTF-8 segments appear only after the first non-whitespace non-ASCII code point and additional non-ASCII whitespace segments appear both before and after that first code point.
