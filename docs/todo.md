...
87. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII diagnostics do not classify mixed tokens when malformed UTF-8 appears before the first non-whitespace non-ASCII code point even if earlier non-ASCII whitespace is present.
