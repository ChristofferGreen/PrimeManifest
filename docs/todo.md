...
94. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks report malformed-UTF-8 diagnostics when multiple malformed segments appear before the first non-whitespace non-ASCII code point even if multiple non-ASCII whitespace segments also appear.
