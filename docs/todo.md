...
104. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII-only checks keep classifying tokens when multiple malformed UTF-8 segments appear only after the first non-whitespace non-ASCII code point and additional non-ASCII whitespace segments appear both before and after that first code point.
