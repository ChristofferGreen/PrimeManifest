...
98. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII-only checks still do not classify tokens when malformed UTF-8 appears before the first non-whitespace non-ASCII code point even if additional malformed segments and non-ASCII whitespace segments appear later.
