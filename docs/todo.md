...
96. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks report malformed-UTF-8 diagnostics when the first non-whitespace non-ASCII code point is followed by multiple malformed segments and later additional non-ASCII whitespace segments.
