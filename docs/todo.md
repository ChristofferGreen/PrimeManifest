...
100. [ ] Add strict-violation reason-token tests verifying combined malformed-UTF-8 and non-whitespace non-ASCII checks report malformed-UTF-8 diagnostics when multiple malformed UTF-8 segments appear before the first non-whitespace non-ASCII code point and additional malformed plus non-ASCII whitespace segments also appear later.
