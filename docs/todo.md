...
99. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII-only checks still do not classify tokens when multiple malformed UTF-8 segments appear before the first non-whitespace non-ASCII code point and additional malformed plus non-ASCII whitespace segments also appear later.
