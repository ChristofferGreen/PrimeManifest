...
92. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII diagnostics still do not classify tokens when malformed UTF-8 appears both before and after the first non-whitespace non-ASCII code point if any malformed segment occurs before that first non-whitespace code point.
