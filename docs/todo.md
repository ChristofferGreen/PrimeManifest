...
97. [ ] Add strict-violation reason-token tests verifying non-whitespace non-ASCII-only checks keep classifying tokens when the first non-whitespace non-ASCII code point is followed by multiple malformed segments and later additional non-ASCII whitespace segments.
