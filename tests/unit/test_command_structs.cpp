...
TEST_CASE("skip_diagnostics_strict_violations_key_value_parse") {
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  "Break\x80"
                  "Gap\xC2"
                  "Split\xC3\xA9RowTotals\xE3\x80\x80\xC2\xA0",
                  parsedViolations,
                  nonAsciiReasonWhitespaceOptions,
                  &parseError),
                "non-ASCII-reason-whitespace mode still classifies tokens as unknown names when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::UnknownReasonName,
                "non-ASCII-reason-whitespace mode reports unknown reason when three malformed segments appear only before the first accented code point while multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "non-ASCII-reason-whitespace mode reports reason field index for three-malformed-only-before-first-accented with multiple-whitespace-before-and-after tokens");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  "Break\x80"
                  "Gap\xC2"
                  "Split\xF0\x9F\x98\x80RowTotals\xC2\xA0\xE3\x80\x80",
                  parsedViolations,
                  nonAsciiReasonWhitespaceOptions,
                  &parseError),
                "non-ASCII-reason-whitespace mode still classifies tokens as unknown names when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::UnknownReasonName,
                "non-ASCII-reason-whitespace mode reports unknown reason when three malformed segments appear only before the first emoji code point while multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "non-ASCII-reason-whitespace mode reports reason field index for three-malformed-only-before-first-emoji with multiple-whitespace-before-and-after tokens");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  "Break\x80"
                  "Gap\xC2"
                  "Split\xC3\xA9RowTotals\xC2\xA0\xE3\x80\x80",
                  parsedViolations,
                  malformedUtf8ReasonOptions,
                  &parseError),
                "malformed-UTF-8 reason-token mode reports malformed diagnostics when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::ReasonNameMalformedUtf8Token,
                "malformed-UTF-8 reason-token mode keeps malformed diagnostics when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "malformed-UTF-8 reason-token mode reports reason field index for three-malformed-only-before-first-accented with surrounding non-ASCII whitespace segments");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  "Break\x80"
                  "Gap\xC2"
                  "Split\xF0\x9F\x98\x80RowTotals\xE3\x80\x80\xC2\xA0",
                  parsedViolations,
                  malformedUtf8ReasonOptions,
                  &parseError),
                "malformed-UTF-8 reason-token mode reports malformed diagnostics when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::ReasonNameMalformedUtf8Token,
                "malformed-UTF-8 reason-token mode keeps malformed diagnostics when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "malformed-UTF-8 reason-token mode reports reason field index for three-malformed-only-before-first-emoji with surrounding non-ASCII whitespace segments");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  nonWhitespaceNonAsciiReasonOptions,
                  &parseError),
                "non-whitespace non-ASCII reason-token mode still leaves tokens as unknown names when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::UnknownReasonName,
                "non-whitespace non-ASCII reason-token mode reports unknown reason when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "non-whitespace non-ASCII reason-token mode reports reason field index for three-malformed-only-before-first-accented with surrounding non-ASCII whitespace segments");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  "Break\x80"
                  "Gap\xC2"
                  "Split\xF0\x9F\x98\x80Total\xE3\x80\x80\xC2\xA0",
                  parsedViolations,
                  nonWhitespaceNonAsciiReasonOptions,
                  &parseError),
                "non-whitespace non-ASCII reason-token mode still leaves tokens as unknown names when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::UnknownReasonName,
                "non-whitespace non-ASCII reason-token mode reports unknown reason when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "non-whitespace non-ASCII reason-token mode reports reason field index for three-malformed-only-before-first-emoji with surrounding non-ASCII whitespace segments");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  malformedAndNonWhitespaceNonAsciiReasonOptions,
                  &parseError),
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode reports malformed-UTF-8 diagnostics when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::ReasonNameMalformedUtf8Token,
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode keeps malformed-UTF-8 diagnostics when three malformed segments appear only before the first accented code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode reports reason field index for three-malformed-only-before-first-accented with surrounding non-ASCII whitespace segments");
  ...
  CHECK_MESSAGE(!parseSkipDiagnosticsStrictViolationsKeyValue(
                  ...
                  malformedAndNonWhitespaceNonAsciiReasonOptions,
                  &parseError),
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode reports malformed-UTF-8 diagnostics when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments appear both before and after that first code point");
  CHECK_MESSAGE(parseError.reason == SkipDiagnosticsParseErrorReason::ReasonNameMalformedUtf8Token,
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode keeps malformed-UTF-8 diagnostics when three malformed segments appear only before the first emoji code point and multiple non-ASCII whitespace segments surround that first code point");
  CHECK_MESSAGE(parseError.fieldIndex == 2,
                "combined malformed-UTF-8 and non-whitespace non-ASCII mode reports reason field index for three-malformed-only-before-first-emoji with surrounding non-ASCII whitespace segments");
