...
enum class SkipDiagnosticsParseErrorReason : uint8_t {
  ...
  ReasonNameSameOrderCesu8SurrogateToken = 45,
};

constexpr size_t SkipDiagnosticsParseErrorReasonCount =
  static_cast<size_t>(SkipDiagnosticsParseErrorReason::ReasonNameSameOrderCesu8SurrogateToken) + 1u;
...
struct SkipDiagnosticsStrictViolationsParseOptions {
  ...
  bool rejectReasonNameSameOrderCesu8SurrogateTokens = false;
...
constexpr auto skipDiagnosticsParseErrorReasonName(SkipDiagnosticsParseErrorReason reason) -> std::string_view {
  switch (reason) {
    ...
    case SkipDiagnosticsParseErrorReason::ZeroPaddedNumericToken:
    ...
    case SkipDiagnosticsParseErrorReason::ReasonNameLoneCesu8SurrogateToken:
    ...
    case SkipDiagnosticsParseErrorReason::ReasonNameSameOrderCesu8SurrogateToken:
      return "ReasonNameSameOrderCesu8SurrogateToken";
  }
...
inline auto isZeroPaddedUnsignedToken(std::string_view text) -> bool {
  return text.size() > 1u && text[0] == '0';
}

inline auto isPlusPrefixedUnsignedToken(std::string_view text) -> bool {
...
inline auto isNegativeZeroToken(std::string_view text) -> bool {
  return text == "-0";
}
...
inline auto isTrailingAsciiWhitespaceUnsignedToken(std::string_view text) -> bool {
  if (text.empty() || !isAsciiWhitespace(text[text.size() - 1u])) return false;

  size_t lastNonWhitespace = text.size();
  while (lastNonWhitespace > 0u &&
         isAsciiWhitespace(text[lastNonWhitespace - 1u])) {
    lastNonWhitespace -= 1u;
  }
  if (lastNonWhitespace == 0u) return false;

  for (size_t index = 0; index < lastNonWhitespace; ++index) {
    char c = text[index];
    if (c < '0' || c > '9') return false;
  }
  return true;
...
inline auto decodeCesu8SurrogateCodeUnit(std::string_view text,
                                         size_t offset,
                                         uint16_t& codeUnitOut) -> bool {
  if (offset + 2u >= text.size()) return false;

  uint8_t b0 = static_cast<uint8_t>(text[offset]);
  uint8_t b1 = static_cast<uint8_t>(text[offset + 1u]);
  uint8_t b2 = static_cast<uint8_t>(text[offset + 2u]);
  if (b0 != 0xEDu ||
      b1 < 0xA0u || b1 > 0xBFu ||
      (b2 & 0xC0u) != 0x80u) {
    return false;
  ...
  char32_t codePoint = static_cast<char32_t>(b0 & 0x0Fu) << 12u;
  codePoint |= static_cast<char32_t>(b1 & 0x3Fu) << 6u;
  codePoint |= static_cast<char32_t>(b2 & 0x3Fu);
  if (codePoint < 0xD800u || codePoint > 0xDFFFu) return false;

  codeUnitOut = static_cast<uint16_t>(codePoint);
  return true;
}

inline auto hasLoneCesu8SurrogateCodeUnits(std::string_view text) -> bool {
  size_t index = 0;
  while (index < text.size()) {
    uint8_t byte = static_cast<uint8_t>(text[index]);
    if (byte < 0x80u) {
    ...
    bool isHighSurrogate = codeUnit >= 0xD800u && codeUnit <= 0xDBFFu;
    if (isHighSurrogate) {
...
inline auto hasPairedCesu8SurrogateCodeUnits(std::string_view text) -> bool {
  size_t index = 0;
  while (index < text.size()) {
    uint8_t byte = static_cast<uint8_t>(text[index]);
    if (byte < 0x80u) {
...
inline auto hasSameOrderCesu8SurrogateCodeUnits(std::string_view text) -> bool {
  size_t index = 0;
  while (index < text.size()) {
    uint8_t byte = static_cast<uint8_t>(text[index]);
    if (byte < 0x80u) {
    ...
    uint16_t nextCodeUnit = 0;
    if (decodeCesu8SurrogateCodeUnit(text, index + 3u, nextCodeUnit)) {
      bool firstHigh = codeUnit >= 0xD800u && codeUnit <= 0xDBFFu;
      bool firstLow = codeUnit >= 0xDC00u && codeUnit <= 0xDFFFu;
      bool nextHigh = nextCodeUnit >= 0xD800u && nextCodeUnit <= 0xDBFFu;
      bool nextLow = nextCodeUnit >= 0xDC00u && nextCodeUnit <= 0xDFFFu;
      if ((firstHigh && nextHigh) || (firstLow && nextLow)) {
        return true;
      }
    }
...
inline auto parseSkipDiagnosticsStrictViolationsKeyValue(
  ...
  while (start <= dump.size()) {
    ...
    if (key == "strictViolations.count") {
      ...
      if (options.rejectTrailingAsciiWhitespaceNumericTokens &&
          isTrailingAsciiWhitespaceUnsignedToken(valueText)) {
        return failSkipDiagnosticsParse(errorOut, fieldIndex, SkipDiagnosticsParseErrorReason::TrailingAsciiWhitespaceNumericToken);
      }
      ...
      if (options.rejectZeroPaddedNumericTokens &&
          isZeroPaddedUnsignedToken(valueText)) {
        return failSkipDiagnosticsParse(errorOut, fieldIndex, SkipDiagnosticsParseErrorReason::ZeroPaddedNumericToken);
      }
      ...
      if (options.rejectTrailingAsciiWhitespaceNumericTokens &&
          isTrailingAsciiWhitespaceUnsignedToken(violationIndexText)) {
        return failSkipDiagnosticsParse(errorOut, fieldIndex, SkipDiagnosticsParseErrorReason::TrailingAsciiWhitespaceNumericToken);
      }
      if (options.rejectNegativeZeroTokens &&
      ...
      if (options.rejectZeroPaddedNumericTokens &&
          isZeroPaddedUnsignedToken(violationIndexText)) {
        return failSkipDiagnosticsParse(errorOut, fieldIndex, SkipDiagnosticsParseErrorReason::ZeroPaddedNumericToken);
      }
      ...
      if (leafKey == "fieldIndex") {
        ...
        if (options.rejectReasonNameSameOrderCesu8SurrogateTokens &&
            hasSameOrderCesu8SurrogateCodeUnits(valueText)) {
          return failSkipDiagnosticsParse(errorOut,
                                          fieldIndex,
        ...
        if (options.rejectReasonNamePairedCesu8SurrogateTokens &&
            hasPairedCesu8SurrogateCodeUnits(valueText)) {
          return failSkipDiagnosticsParse(errorOut,
                                          fieldIndex,
        ...
        if (options.rejectReasonNameNonWhitespaceNonAsciiTokens &&
            isNonWhitespaceNonAsciiReasonNameToken(valueText)) {
          return failSkipDiagnosticsParse(errorOut, fieldIndex, SkipDiagnosticsParseErrorReason::ReasonNameNonWhitespaceNonAsciiToken);
        }
        SkipDiagnosticsParseErrorReason parsedReason{};
